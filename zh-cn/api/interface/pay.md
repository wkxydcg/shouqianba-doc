...
extended | 扩展参数集合 | JSON map | N | 收钱吧与特定第三方单独约定的参数集合,json格式，最多支持24个字段，每个字段key长度不超过64字节，value长度不超过256字节,支付宝花呗参数放在这里进行透传hb_fq_num:分期数,hb_fq_seller_percent:商家承担手续费比例。sqb_hb_fq_buyer_service_charge 卖家承担的手续金额(分),sqb_hb_fq_seller_service_charge:卖家承担的手续费金额(分)| {"goods_tag":"beijing","extend_params":{"hb_fq_num":"12","hb_fq_seller_percent":"0"},"sqb_hb_fq_buyer_service_charge":100,"sqb_hb_fq_seller_service_charge":0},
